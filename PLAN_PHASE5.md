...
### 5.3 Cheetah adapter (1 week) — done 2026-09-05
- [x] Env `cheetah` (python 3.11, torch CPU, `cheetah-accelerator`); GPL: `lattix/formats/cheetah.py` is an adapter (imported at call time, absent from `FORMATS`, like HELIX).
- [x] `to_segment(lattice, species)` / `from_segment(segment)`: table over the 22 kinds — Drift, Quadrupole (k1, tilt, misalignment), Sextupole, Bend/RBend (e1/e2, gap, fint), Solenoid (k = ks), RFCavity → `Cavity(voltage = −V/sign q, phase° , frequency)`, thin cavity as `length = 0` if supported (*verify*), Kicker → `HorizontalCorrector`+`VerticalCorrector` (or `CombinedCorrector`), Collimator → `Aperture`, Marker/Instrument → `Marker`/`BPM`/`Screen`, Taylor → `CustomTransferMap` (7×7 with the IR 6×6 embedded), FieldMap → `Cavity` via `FM_TO_CAVITY`, NCells/RFQCell → Drift (LOSSY), Patch/ReferenceChange/Freq/Directive → DROPPED with codes, Superposition → flattened.
- [x] Oracle `lattix/oracles/cheetah.py`: `Segment.first_order_transfer_map(energy, species)` per element (7×7 → 6×6 + kick), fingerprint (τ late-positive expected), p0 follows cavities.
- [x] Gate: `fodo.madx` vs cpymad T4×4 1e-8; MEBT quads + gaps vs HELIX Equivalent tier (Cheetah's cavity matrix differs from the thin-gap model by design; record `CHEETAH_CAVITY_MODEL`).
- Delivered as a format (`lattix/formats/cheetah/`, LatticeJSON, pure Python — Cheetah is GPL and never imported) plus an out-of-process oracle; measured: τ late-positive (`Basis.CHEETAH`), `phase = −φ`, `voltage = −V/q`, charge-blind k1/k (signed rigidity), zero-length cavity = inf (1 µm in the oracle, the lattix thin-gap lens carries the focusing). Gates: fodo vs cpymad 1.8e-15 (all blocks ≤ 8e-9), MEBT vs HELIX 6.5e-3, solenoid channel 5e-15; lockstep: Cheetah's own Bmad converter on lattix's `.bmad` agrees with lattix's LatticeJSON below 1e-8.

### 5.4 PyORBIT3 linac XML (1.5 weeks incl. build) — done 2026-09-05
- [x] Env `pyorbit` (conda py 3.10 + meson build of the clone at `22b45fa`, `USE_MPI=none`; the worker preloads `libfftw3` on macOS); `lattix/formats/pyorbit/` reader + writer: one `<seq>` per lattice, `pos` = centre positions, QUAD field T/m, BEND theta/ea1/ea2, RFGAP `E0TL` (GeV) + `phase` (deg) with **one `<Cavity>` per gap** (PyORBIT derives a shared cavity's later gaps from the time of flight — grouping by frequency lost 2.6 % of the MEBT energy), DCH/DCV from Kicker (a thick one kicks at its centre between `_in`/`_out` markers the reader folds back), SOLENOID `B₀/Bρ`, MARKER for Instrument/Marker/Collimator/Foil (a VACWIN has no linear-optics meaning), apertures as full diameters. Thin nodes inside thick magnets split them on read, as PyORBIT applies them between the parts.
- [x] Oracle: `LinacTrMatricesController` nodes at every node entrance (cumulated maps, per-node = `R[k+1]·R[k]⁻¹`, exit node added), 13-particle symmetric probes tracked at two amplitudes and Richardson-extrapolated (the tracker's third-order terms left 5e-5 on the chicane at ±1e-5); fingerprint (`R56 = +237.30`, gain 866 025.4 eV, bunching slope); species via `bunch.mass()`/`charge()`. Ring MAD8 lattices are lattix's MAD8 reader's job, not this format's.
- [x] Gate: SNS MEBT (vendored `<MEBT>` of `sns_linac.xml`) and ESS MEBT read → write fixed points, PyORBIT on original vs rewrite 7.7e-10 / 6.6e-11 with identical energies; `.dat` → XML vs HELIX: quads 1.2e-11, solenoid 9e-13, bends 2.9e-11, chicane 1.1e-8 (Exact), MEBT thin gaps 4.3e-3 with the energy exact (Equivalent, PyORBIT's own gap focusing).

### 5.5 IMPACT-T (1.5 weeks) — done 2026-09-05
- [x] `lattix/formats/impactt/`: `zedge` positioning from `s_in`, types 0/1/3/4/5/104 written (a solenoid needs an `(r, z)` table — designed so the tracked map is the hard edge's to O(h²); a dipole the 22-value pole-face file; a cavity an `rfdataN` Fourier profile no wider than βλ/2 or an n-cell train, `theta0` calibrated by integrating the reference; a thin gap a short surrogate cavity whose length balances kick smearing against ponderomotive focusing), controls `-1/-2/-11` for kickers, dumps and collimators, `-99` end; reader inverse (timeline by `zedge`, implicit gaps, overlaps, thin-inside-thick, thin-gap and solenoid restoration from the lattix tags, passthrough of the other RF types with their data files).
- [x] Engine: conda-forge `impact-t` 3.1.5 run directly (`ImpactTexe`; no `lume-impact`); oracle: `-2` dumps (fixed-time snapshots drifted to the reference plane) and `-4` per-element steps at every boundary, `partcl.data` probes, chunking at 90 controls, dipole frame re-basing; basis measured (ahead-positive z, γβz) and fingerprinted; marker `oracle_impactt`.
- [x] Gate: `fodo.madx` quads/drifts 1.2e-9 vs cpymad (bends 1e-2: IMPACT-T's dipole has no pole-face focusing — report-only); `fodo_cell` 2.3e-8 and `solenoid_channel` 9.3e-7 vs HELIX; MEBT gaps: reference energy 1.5e-7 vs HELIX, transverse 8.1e-3 (the profile's RF focusing; the IMPACT-Z leg was not run — both codes take different profile models); thin-gap gain 3.3e-6 for both charges, thick cavities 7e-7; `Sample1` reads and rewrites bit-exact with its data files.

### 5.6 Bmad bridge (3 days) — done 2026-09-05
- [x] `lattix convert --to astra|gpt|sad|csrtrack|merlin|slicktrack` = IR → `.bmad` in a temp dir → `bmad_to_astra` / `bmad_to_gpt` / `bmad_to_csrtrack` (namelist inputs generated by lattix) / `bmad_to_merlin` / `bmad_to_slicktrack` / Tao `write sad` in env `bmad`; ledger `EQUIVALENT VIA_BMAD` per element plus Bmad's own conversion notes captured from stdout (`BMAD_CONVERTER_NOTE`, `BMAD_CONVERTER_LOSS`). Not bridged (measured): OPAL and XSIF — the Tao of Bmad 20260828 accepts no `write opal|opal_latice|xsif`; GPT has no bend translation (Bmad says so, recorded as a loss).
...
### 5.11 Cross-cutting — done 2026-09-06
- [x] CI: `oracles.yml` runs `lightwin` (py 3.12), `cheetah` (torch CPU), `ocelot` and `pyorbit` (meson build, pinned commit) as separate jobs; `impact-t=3.1.5` pinned in `environment-ci.yml`; no GPL package (Cheetah, Ocelot, OPAL) is imported at module level or vendored (engines run out of process in their own envs; DYNAC and Synergia are local-only); Bmad bridge tests run in the bmad env.
- [x] Docs: a page per format under `docs/formats/` (enforced by `tests/docs`), `docs/oracles.md` rows and Phase 5.x measurement sections with fingerprints for every engine, conventions rows.
- [x] Ledger codes: every writer (20 formats with a writer) has a `check_rules_coverage` test; `docs/fidelity.md` is regenerated by the catalogue (586 codes).
...
5.0 SciBmad (done) → 5.1 xsuite (done) → 5.2 LightWin (done) → 5.3 Cheetah (done) → 5.4 PyORBIT3 (done) → 5.5 IMPACT-T (done) → 5.6 Bmad bridge (done) → 5.7 Ocelot (done) → 5.8 DYNAC (done) → 5.9 Synergia2 (done) → 5.10 OPAL-T (done) → 5.11 cross-cutting (done). Phase 5 complete 2026-09-06.
