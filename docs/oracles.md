...
| HELIX | in-process, `HELIX_ROOT` (required; there is no default checkout); MAD-X/MAD8/Elegant decks go through lattix's reader and TraceWin writer first (HELIX's own MAD-X parser does not follow `call, file=`) | linac_gen 1.9.1 | (x mm, x′ mrad, y mm, y′ mrad, Δφ deg, ΔW MeV) | follows | Δφ is *late-positive* (z = −βλ/360·Δφ); ΔW → δ = ΔW/(β²γmc²). **HELIX dipoles before f0c37e5 omit the path-length row (R51, R52) and momentum compaction (R56); f0c37e5 (2026-09-08) adds them and the oracle reports `dipole_path_row`, so the battery compares those blocks only on a tree that has them** |
...
| OPAL-T | none — OPAL-X `078ff1e` is read for its conventions only (GPL-3; a build needs MPI, H5hut, Boost, GSL); Tao's `write opal` cannot serve as a reference (`tao_write_cmd.f90` dispatches on the misspelt `opal_latice`) | `lattix/formats/opal/` (no adapter) | (x, px, y, py, t, pz) time integration | follows the fields | conventions source-derived; the battery runs OPAL as an engine-less format (fixed points, IR round trips) |
...
* **TraceWin's bend sign, measured with the licensed binary**: `BEND −11.25 ρ>0` gives the
  ...
  **Fixed in HELIX d3f281a (2026-09-06)**: `psb.seq` (32 negative bends) now agrees on T4x4 = 2.3e-13 and
  dispersion 1.7e-13; what remains (path 5.0e2, R56 1.1e3) is the dipole's longitudinal row, which HELIX's
  manual (`docs/manual/03_elements/04_dipole.md`) documents as a drift's — no R51/R52, no momentum
  compaction. The oracle records `helix_commit` and `dipole_negative_bend_fixed` (git ancestry of d3f281a)
  and the verdict keeps the report-only rule only for an older tree.
  **Fixed in HELIX f0c37e5 (2026-09-08)**: the dipole now carries R51/R52 and the momentum compaction; on
  `fodo.madx` the path block agrees with MAD-X to 1.9e-16. The oracle records `dipole_path_row` the same way
  and the verdict drops path/R56 for HELIX only when that flag is not True.
...
## Phase 5.10 notes (OPAL-T, 2026-09-06)

* No engine: OPAL-X's source fixes the conventions (`docs/formats/opal.md`): strengths over the BEAM's
  initial unsigned `P0/c`, `LAG` relative to the autophased crest, `DESIGNENERGY` as the crest kinetic
  energy in MeV, `ELEMEDGE` placement, maps whose extent is the element length.
* Bmad/Tao cannot write a reference deck: `tao/code/tao_write_cmd.f90` lists `opal` among the `write`
  keywords but the `select case` matches `'opal_latice'`, so both `write opal` and `write opal_latice` fail
  (`UNKNOWN "WHAT"` / `UNRECOGNIZED "WHAT"`); reported in `docs/formats/opal.md`.  Its writer would also
  disagree with the OPAL-X source on `k1` (lab gradient vs normalized).
...
| HELIX | 7.1e-11 | 4.7e-11 | exact; T4x4/disp need HELIX ≥ d3f281a, path/R56 are compared from f0c37e5 on; `psb.seq` 2.9e-16 |
